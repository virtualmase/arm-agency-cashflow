...
function SwellEditorialReviewQueue() {
  const utils = trpc.useUtils();
  const { data: monitor } = trpc.admin.swellEditorial.monitor.useQuery();
  const { data: reviews, isLoading } = trpc.admin.swellEditorial.reviews.useQuery({ limit: 50 });
  const [notes, setNotes] = useState<Record<number, string>>({});
  const updateReview = trpc.admin.swellEditorial.updateReview.useMutation({
    onSuccess: () => { toast.success("Editorial review status updated"); utils.admin.swellEditorial.reviews.invalidate(); },
    onError: (error) => toast.error(error.message),
  });
  const statuses: Record<string, string> = {
    pending_review: "text-[#e8a020] border-[#a06010]", approved: "text-[#3ddc84] border-[#1a7040]", declined: "text-[#ff8080] border-[#ff4444]/30", published: "text-[#a78bfa] border-[#7b5ea7]", expired: "text-[#667066] border-white/[0.12]",
  };
  const renderRecommendations = (value: string | null | undefined, empty: string) => {
    if (!value) return <p className="text-[11px] text-[#667066]">{empty}</p>;
    try {
      const items = JSON.parse(value) as Array<{ title?: string; label?: string; url?: string; why?: string; reason?: string }>;
      return <ul className="space-y-1.5 text-[11px] text-[#aab3aa]">{items.map((item, index) => <li key={`${item.url}-${index}`}><a href={item.url} target="_blank" rel="noreferrer" className="text-[#3ddc84] hover:text-[#e8a020]">{item.title || item.label || item.url}</a><span className="text-[#667066]"> — {item.why || item.reason}</span></li>)}</ul>;
    } catch { return <p className="text-[11px] text-[#667066]">Review structured recommendations in the private record.</p>; }
  };
  return <section className="mt-8 p-6 bg-[#0d100d] border border-white/[0.07]">
    <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4 mb-6"><div><div className="text-[10px] tracking-[0.15em] uppercase text-[#667066] mb-1">Swell publication response queue</div><div className="text-lg font-light text-[#eaf0ea]">Private editorial review</div></div><p className="text-[11px] leading-[1.6] text-[#667066] max-w-lg lg:text-right">Sitemap detection creates owner-only briefs. Generated text is never approved public copy; source attribution, citations, links, claims, and publication each require review.</p></div>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5"><div className="p-3 bg-[#0b1210] border border-white/[0.05]"><div className="text-[9px] tracking-[0.12em] uppercase text-[#667066]">Monitor</div><div className="mt-1 text-[12px] text-[#eaf0ea]">{monitor?.enabled ? "Daily review active" : "Awaiting activation"}</div></div><div className="p-3 bg-[#0b1210] border border-white/[0.05]"><div className="text-[9px] tracking-[0.12em] uppercase text-[#667066]">Retention</div><div className="mt-1 text-[12px] text-[#eaf0ea]">{monitor?.retentionDays ?? 90} days</div></div><div className="p-3 bg-[#0b1210] border border-white/[0.05]"><div className="text-[9px] tracking-[0.12em] uppercase text-[#667066]">Last run</div><div className="mt-1 text-[12px] text-[#eaf0ea]">{monitor?.lastCheckedAt ? new Date(monitor.lastCheckedAt).toLocaleString() : "No completed run"}</div></div></div>
    {monitor?.lastCheckSummary && <p className="mb-5 text-[11px] text-[#667066]">{monitor.lastCheckSummary}</p>}
    {isLoading ? <div className="h-32 animate-pulse bg-[#111411]" /> : !reviews?.length ? <div className="py-8 text-center text-[13px] text-[#667066]">No private reviews yet. New Swell resource versions will appear after the daily monitor runs.</div> : <div className="space-y-4">{reviews.map((review) => {
      let brief: { buyerDecision?: string; originalAngle?: string; brief?: string } = {};
      try { brief = JSON.parse(review.generatedBrief); } catch { brief = { brief: review.generatedBrief }; }
      return <article key={review.id} className="p-4 bg-[#0b1210] border border-white/[0.05]"><div className="flex flex-col lg:flex-row lg:justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2 mb-2"><span className={`text-[9px] tracking-[0.12em] uppercase px-2 py-0.5 border ${statuses[review.status] || statuses.pending_review}`}>{review.status.replace("_", " ")}</span><span className="text-[11px] text-[#667066]">Detected {new Date(review.detectedAt).toLocaleDateString()}</span></div><h3 className="text-[15px] text-[#eaf0ea]">{review.sourceTitle}</h3><a href={review.sourceUrl} target="_blank" rel="noreferrer" className="inline-block mt-1 text-[11px] text-[#3ddc84] hover:text-[#e8a020] break-all">Original Swell source ↗</a></div></div>{review.sourceDescription && <p className="mt-3 text-[12px] leading-[1.65] text-[#aab3aa]">Source context: {review.sourceDescription}</p>}<div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-4"><div><div className="text-[9px] tracking-[0.12em] uppercase text-[#667066]">Private ARM angle</div><p className="mt-1 text-[13px] leading-[1.7] text-[#c8cfc8]">{brief.originalAngle || "No angle recorded."}</p><div className="mt-3 text-[9px] tracking-[0.12em] uppercase text-[#667066]">Buyer decision</div><p className="mt-1 text-[12px] leading-[1.7] text-[#aab3aa]">{brief.buyerDecision || "No decision recorded."}</p><div className="mt-3 text-[9px] tracking-[0.12em] uppercase text-[#667066]">Private brief</div><p className="mt-1 text-[12px] leading-[1.7] text-[#aab3aa] whitespace-pre-line">{brief.brief || "No brief recorded."}</p></div><div><div className="text-[9px] tracking-[0.12em] uppercase text-[#667066]">Research leads to verify</div><div className="mt-2">{renderRecommendations(review.suggestedSources, "No research leads recorded.")}</div><div className="mt-4 text-[9px] tracking-[0.12em] uppercase text-[#667066]">Contextual property links</div><div className="mt-2">{renderRecommendations(review.suggestedLinks, "No property links recorded.")}</div><div className="mt-4 text-[9px] tracking-[0.12em] uppercase text-[#667066]">Claim and approval notes</div><p className="mt-1 text-[12px] leading-[1.7] text-[#e8a020]">{review.claimNotes || "No claim notes recorded."}</p></div></div>{review.status === "pending_review" && <div className="mt-4 flex flex-col md:flex-row gap-3"><textarea value={notes[review.id] || ""} onChange={(event) => setNotes((current) => ({ ...current, [review.id]: event.target.value }))} placeholder="Owner review notes (optional)" rows={2} className="flex-1 bg-transparent border border-white/[0.07] px-3 py-2 text-[12px] text-[#eaf0ea] outline-none focus:border-[#e8a020] resize-none" /><button onClick={() => updateReview.mutate({ id: review.id, status: "approved", reviewNotes: notes[review.id] || undefined })} disabled={updateReview.isPending} className="px-4 bg-[#3ddc84] text-[#080a08] text-[10px] font-semibold tracking-[0.12em] uppercase disabled:opacity-50">Approve brief</button><button onClick={() => updateReview.mutate({ id: review.id, status: "declined", reviewNotes: notes[review.id] || undefined })} disabled={updateReview.isPending} className="px-4 border border-[#ff4444]/40 text-[#ff8080] text-[10px] font-semibold tracking-[0.12em] uppercase disabled:opacity-50">Decline</button></div>}</article>;
    })}</div>}
  </section>;
}
