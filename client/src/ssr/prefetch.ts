...
export async function prefetchForPath(url: string, _queryClient: QueryClient, _prefetch: SsrPrefetch): Promise<HeadMeta> {
  ...
  const insightMeta: Record<string, HeadMeta> = {
    ...
    "/insights/agent-payment-readiness": { title: `Agent Payment Readiness Score for x402 and AiFi · ${SITE}`, description: "A scored stop, sandbox, or bounded-pilot instrument for teams enabling agent payments and financial autonomy.", canonicalPath: "/insights/agent-payment-readiness", imagePath: "/manus-storage/agent-payment-readiness-five-gates_77b5971d.png", imageAlt: "Five architectural verification gates on a bounded agent-payment path" },
