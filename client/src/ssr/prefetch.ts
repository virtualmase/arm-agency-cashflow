...
export async function prefetchForPath(url: string, _queryClient: QueryClient, _prefetch: SsrPrefetch): Promise<HeadMeta> {
  ...
  const insightMeta: Record<string, HeadMeta> = {
    ...
    "/insights/agent-payment-readiness": { title: `Agent Payment Readiness for x402 and AiFi · ${SITE}`, description: "A production-readiness gate for teams enabling agent payments and bounded financial autonomy.", canonicalPath: "/insights/agent-payment-readiness" },
