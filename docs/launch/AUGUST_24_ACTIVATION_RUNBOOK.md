...
| 2026-08-19 07:36 UTC | Support forwarding route | Owner confirmed a Hostinger forwarder from `ops@arm-agency.xyz` to the operating inbox `ops@arm-agency.com`. A labeled external message titled `[ARM CONTROLLED LAUNCH] Support forwarding verification` was accepted for delivery to `ops@arm-agency.xyz`; the owner must retain the receiving-mailbox confirmation as the final receipt evidence. | Connected mail-service sent record; owner-provided forwarding confirmation | Client-success owner | Publish the corrected public `ops@arm-agency.xyz` contact detail. Do not treat this forwarder test as proof that the website can send transactional mail: the application sender remains disabled until a provider is configured. |
| | | | | | |
